...
public class EnemyAI : State {
	...
	//The wavepoint we are currently moving towards
	...
	IEnumerator SearchForPlayer() {
		GameObject sReslt = GameObject.FindGameObjectWithTag ("Player");
		if (sReslt == null) {
			yield return new WaitForSeconds (0.5f);
			StartCoroutine (SearchForPlayer());
		} else {
			searchingForPlayer = false;
			target = sReslt.transform;
			StartCoroutine (UpdatePath ());
			yield return false;
		}
	}
